...
/* Define AES implementation includes and functions */
    ...
    #ifdef FREESCALE_LPC_AES_GCM
        #undef NEED_AES_TABLES
        #undef GCM_TABLE
    #endif /* FREESCALE_LPC_AES_GCM */
    
    /* LPC doesn't have CCM, use software with LPC AES ECB mode */
    static int wc_AesEncrypt(Aes* aes, const byte* inBlock, byte* outBlock)
    {    
        wc_AesEncryptDirect(aes, outBlock, inBlock);
        return 0;
...
/* AES-DIRECT */
        ...
        void wc_AesEncryptDirect(Aes* aes, byte* out, const byte* in)
        {
            ...
            AES_SetKey(AES_BASE, key, keySize);
            AES_EncryptEcb(AES_BASE, in, out, 16);
        }
        ...
        void wc_AesDecryptDirect(Aes* aes, byte* out, const byte* in)
        {
            ...
            AES_SetKey(AES_BASE, key, keySize);
            AES_DecryptEcb(AES_BASE, in, out, 16);
        }
    #elif defined(STM32F2_CRYPTO) && defined(HAVE_AES_DECRYPT)
...
/* AES-CBC */
    ...
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
        ...
        status = AES_SetKey(AES_BASE, enc_key, keySize);
		if (status != kStatus_Success)
            return -1;

        status = AES_EncryptCbc(AES_BASE, in, out, sz, iv);
        if (status == kStatus_Success)
            return 0;
        else
            return -1;
    ...
    int wc_AesCbcDecrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
        ...
        status = AES_SetKey(AES_BASE, dec_key, keySize);        
        if (status != kStatus_Success)
            return -1;
        
        status = AES_DecryptCbc(AES_BASE, in, out, sz, iv);                
        if (status == kStatus_Success)
            return 0;
        else
            return -1;
...
/* AES-CTR */
        ...
        void wc_AesCtrEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
        {
            ...
            if (sz)
            {
                ...
                AES_SetKey(AES_BASE, enc_key, keySize);
                AES_CryptCtr(AES_BASE, in, out, sz, iv, (byte*)aes->tmp, (uint32_t*)&(aes->left));
            }
        }
...
#if defined (FREESCALE_LTC_AES_GCM) || defined(FREESCALE_LPC_AES_GCM)
#else
                   ...
                   const byte* authIn, word32 authInSz)
{
    ...
    status = AES_SetKey(AES_BASE, key, keySize);
    if (status != kStatus_Success)
        return AES_GCM_AUTH_E;
    
    status = AES_EncryptTagGcm(AES_BASE,
                               in,
                               out,
                               sz,
                               iv,
                   ...
                   const byte* authIn, word32 authInSz)
{
    ...
    status = AES_DecryptTagGcm(AES_BASE,
                               in,
                               out,
                               sz,
                               iv,
                   ...
                   const byte* authIn, word32 authInSz)
{
    ...
    status_t retVal;
    retVal = LTC_AES_DecryptTagCcm(LTC_BASE,
                                         in,
                                         out,
                                         inSz,
                                         nonce,
